...
const fs = require('fs');
const ProfileView = require('./models/Interest.js'); // Ensure this path is correct
...
const migrateProfileViews = async () => {
    try {
        const rawData = fs.readFileSync('./expressinterest.json'); // Adjust the path as needed
        ...
        for (const oldView of oldProfileViews) {
            if (oldView.date && oldView.who && oldView.whom) {
                const newView = {
                    interestedUserId: oldView.who,
                    targetUserId: oldView.whom,
